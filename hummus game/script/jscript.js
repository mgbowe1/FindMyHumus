...
//when choosing a utensil
    ...
    ingredients.on('click', function () {
        ...
        else if(askedIngredient!==$('.selected').attr('data-can-scoop')){
            $('.selected').addClass('wrong-tool');
            setTimeout(function () {
                $('.selected').removeClass('wrong-tool');
            },200);
        }
        setTimeout(function () {
            $('#client').css('background-image','url("./images/ninja-character-2.png")');
        },4000);
        setTimeout(function () {
            failed();
        },8000);
    ...
    //     var hummus=Math.floor((Math.random()*4)+1);
    //     var orderObject={egg:egg, hummus:hummus, ful:ful};
    //     var beginOrderObject={egg:0, hummus:0, ful:0};
    //     $('.first-line').text('egg: '+egg);
    //     $('.second-line').text('Hummus: '+hummus);
    //     $('.third-line').text('Ful: '+ful);
    //
    ...
    //         onThePlate = {hummus:0, ful:0, egg:0, parsely:0};
